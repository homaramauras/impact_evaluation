...
provida_df <- read_dta("/Users/homi/Library/CloudStorage/Box-Box/PROVIDA/01_data/02_build/Final_PROVIDA_data.dta") %>%
  filter(!is.na(rent_charge)) %>%
  mutate(col_21 = as.factor(col_21))

# Create dummy variables
col_dummies <- model.matrix(~ col_21 - 1, data = provida_df)
provida_df <- provida_df %>%
  dplyr::select(-col_21) %>%
  bind_cols(as_tibble(col_dummies))

# Define DL covariates
dl_vars <- c("gender", "hh_men_min_21", "housing_index_stand", grep("^col_21", names(provida_df), value = TRUE))
...
all_vars <- all_vars[sapply(provida_df[all_vars], function(x) (is.numeric(x) | is.factor(x)) && all(is.finite(x)) && !any(is.na(x)))]

# CEM function
run_cem <- function(data, outcome, match_vars, label) {
  cem_result <- cem("treat", data = dplyr::select(data, treat, all_of(match_vars)), keep.all = TRUE)
  data <- data %>% mutate(cem_weight = cem_result$w)
  matched <- data %>% filter(!is.na(cem_weight) & cem_weight > 0)
  
  att <- with(matched, weighted.mean(get(outcome)[treat == 1], cem_weight[treat == 1]) -
...
# Run and display
dl_result  <- run_cem(provida_df, "rent_charge", dl_vars, "CEM (DL Covariates)")
all_result <- run_cem(provida_df, "rent_charge", all_vars, "CEM (All Covariates)")
